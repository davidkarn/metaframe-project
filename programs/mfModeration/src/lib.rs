...
\"scorer\":[\"block\",
  ...
  [\"if\", \"scores\",
   ...
   0]],
\"upvote\":[\"block\",
  [\"set\", [\"lst\", \"vote-addr\", \"vote-bump\"],
            [\"pda\", [\"lst\", [\"str\", \"votes\"], \"wallet-key\", \"comment-id\"],
                      \"program-id\"]],
...
\"downvote\":[\"block\",
  ...
  [\"if\", \"acct\",
   ...
   [\"call-program\", 
    ...
    pub fn upvote(ctx: Context<Upvote>, comment_id: String, voter: Pubkey, vote_bump: u8) -> ProgramResult {
    ...
    pub fn downvote(ctx: Context<Downvote>, comment_id: String, voter: Pubkey, vote_bump: u8) -> ProgramResult {
        let vote: &mut Account<Vote>   = &mut ctx.accounts.vote;
        let votes: &mut Account<Votes> = &mut ctx.accounts.votes;
        let author: &Signer            = &ctx.accounts.author;
    ...
    pub fn upvote_new(ctx: Context<UpvoteNew>, comment_id: String, voter: Pubkey, vote_bump: u8, votes_bump: u8) -> ProgramResult {
        let vote: &mut Account<Vote>   = &mut ctx.accounts.vote;
        let votes: &mut Account<Votes> = &mut ctx.accounts.votes;
        let author: &Signer            = &ctx.accounts.author;
        ...
        votes.comment_id  = comment_id;
        votes.up_count    = 1;
        votes.down_count  = 0;
        
        Ok(()) }

    pub fn downvote_new(ctx: Context<DownvoteNew>, comment_id: String, voter: Pubkey, vote_bump: u8, votes_bump: u8) -> ProgramResult {
        let vote: &mut Account<Vote>   = &mut ctx.accounts.vote;
        let votes: &mut Account<Votes> = &mut ctx.accounts.votes;
        let author: &Signer            = &ctx.accounts.author;
...
#[derive(Accounts)]
#[instruction(comment_id: String, bump: u8)]
pub struct UpvoteNew<'info> {
    #[account(init, payer=author, space=Vote::LEN,
              seeds=[b"vote", author.key.as_ref(), comment_id.as_bytes()], bump)]
